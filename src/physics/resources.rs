...
where
    T: for<'w, 's, 'b, 'c, 'g, 'h, 'e, 'f> PhysicsHooks<
            RigidBodyComponentsSet<'w, 's, 'b, 'c>,
            ColliderComponentsSet<'g, 'h, 'e, 'f>,
        > + Send
        + Sync,
    UserData: WorldQuery,
{
...
pub(crate) struct PhysicsHooksWithQueryInstance<'w, 's, 'b, UserData: WorldQuery> {
    pub user_data: Query<'w, 's, UserData>,
    pub hooks: &'b dyn PhysicsHooksWithQuery<UserData>,
}

impl<'ww, 'ss, 'bb, 'w, 's, 'b, 'c, 'g, 'h, 'e, 'f, UserData: WorldQuery>
    PhysicsHooks<RigidBodyComponentsSet<'w, 's, 'b, 'c>, ColliderComponentsSet<'g, 'h, 'e, 'f>>
    for PhysicsHooksWithQueryInstance<'ww, 'ss, 'bb, UserData>
{
    fn filter_contact_pair(
        &self,
        context: &PairFilterContext<RigidBodyComponentsSet, ColliderComponentsSet>,
