...
macro_rules! impl_component_set_mut(
    ($ComponentsSet: ident, $T: ty, |$data: ident| $data_expr: expr) => {
        impl<'w, 's, 'b, 'c> ComponentSetOption<$T> for $ComponentsSet<'w, 's, 'b, 'c> {
            #[inline(always)]
        ...
        impl<'w, 's, 'b, 'c> ComponentSet<$T> for $ComponentsSet<'w, 's, 'b, 'c> {
            #[inline(always)]
        ...
        impl<'w, 's, 'b, 'c> ComponentSetMut<$T> for $ComponentsSet<'w, 's, 'b, 'c> {
            #[inline(always)]
...
pub type ComponentSetQueryMut<'w, 's, 'b, 'c, T> =
    QuerySet<(Query<'w, 's, (Entity, &'b T)>, Query<'w, 's, (Entity, &'c mut T)>)>;

pub struct QueryComponentSetMut<'w, 's, 'b, 'c, T: 'static + Send + Sync>(
    ComponentSetQueryMut<'w, 's, 'b, 'c, T>,
);

impl<'w, 's, 'b, 'c, T: 'static + Send + Sync> ComponentSetOption<T>
    for QueryComponentSetMut<'w, 's, 'b, 'c, T>
{
    #[inline(always)]
    fn get(&self, handle: Index) -> Option<&T> {
        self.0.q0().get_component(handle.entity()).ok()
...
impl<'w, 's, 'b, 'c, T: 'static + Send + Sync> ComponentSet<T> for QueryComponentSetMut<'w, 's, 'b, 'c, T> {
    #[inline(always)]
    fn size_hint(&self) -> usize {
        0
    }
...
impl<'w, 's, 'b, 'c, T: 'static + Send + Sync> ComponentSetMut<T>
    for QueryComponentSetMut<'w, 's, 'b, 'c, T>
{
    #[inline(always)]
    fn set_internal(&mut self, handle: Index, val: T) {
